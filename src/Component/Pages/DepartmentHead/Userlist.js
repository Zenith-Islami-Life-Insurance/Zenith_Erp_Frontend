...
const Userlist = () => {
  ...
  const { PERSONALID, DEPT_NAME, DEPT_CODE, NAME} = userDetailsString;
  ...
  const dept_permission_list = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/permission-list-depthead/${DEPT_CODE}`
      );
  ...
  return (
    <div>
      ...
      <h1 className="shadow w-1/5 mx-auto p-3 mt-5 font-bold rounded text-center">
        {DEPT_NAME} ALL USER PERMISSION LIST
      </h1>
